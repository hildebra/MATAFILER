...
my $scaffSiz;    # scaff's shorter than this will be excluded for the stats
...
GetOptions ("limit=i"       => \$limit,
			...
			"scaff_size=i"  => \$scaffSiz);

# set defaults
$limit = 100000000000 if (!$limit);
$n_limit = 25       if (!$n_limit);
$scaffSiz = 0      if (!$scaffSiz);
...
foreach (('scaffold','contig', 'intermediate')){$data{$_}{lengths} = [];}
...
sub sequence_statistics{
	...
	if($type eq 'intermediate'){
		my $total_size = 0;
		$total_size = sum(@{$data{scaffold}{lengths}}) if ($count>0);
		
		# now calculate percentage of assembly that is accounted for by scaffolded contigs
		my $percent = sprintf("%.1f",0);
		$percent = sprintf("%.1f",($scaffolded_contig_length / $total_size) * 100) if ($total_size>0);
		...
		$percent = sprintf("%.1f",($unscaffolded_contig_length / $total_size) * 100) if ($total_size>0);
		...
		my $average_contigs_per_scaffold = sprintf("%.1f",0);
		$average_contigs_per_scaffold = sprintf("%.1f",$contig_count / $scaffold_count) if ($scaffold_count>0);
	...
	foreach my $size (1000,10000,100000,1000000,10000000){
		my $matches = grep { $_ > $size } @{$data{$type}{lengths}};
		my $percent = sprintf("%.1f", 0 * 100);
		$percent = sprintf("%.1f", ($matches / $count) * 100) if ($count>0);
	...
	# mean sequence size
	my $mean = sprintf("%.0f",0);
	$mean = sprintf("%.0f",$total_size / $count) if ($count>0);
	$desc = "Mean $type size";
	printf "%${w}s %10d\n", $desc, $mean;
    ...
    my $median = 0;
    $median = (sort{$a <=> $b} @{$data{$type}{lengths}})[$count/2]if ($count>0);
...
#	$ng_values[100] = $min;
	...
	my %bases;
	my $seq ="";
    $seq = join('',@{$data{$type}{seqs}}) if ($count>0);  
	my $length = length($seq);
	...
	foreach my $base ("A", "C", "G", "T", "N"){
		my $percent = sprintf("%.2f", 0);
		$percent = sprintf("%.2f", ($bases{$base} / $length) * 100) if ($length>0);  
		$desc = "$type %$base";
		printf "%${w}s %10s\n", $desc, $percent;
		store_results($desc, $percent) if ($csv);
	...
	my $percent = sprintf("%.2f", 0) ;
	 $percent = sprintf("%.2f", ($other / $length) * 100) if ($length>0);  
	$desc = "$type %non-ACGTN";
	printf "%${w}s %10s\n",$desc, $percent;
	store_results($desc, $percent) if ($csv);
